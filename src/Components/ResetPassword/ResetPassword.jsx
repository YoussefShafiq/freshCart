...
export default function ResetPassword() {
  ...
  let validationSchema = Yup.object().shape({
    ...
    newPassword: Yup.string().matches(/^[A-Z]\w{3,20}$/, 'invalid password').required('password is required'),
  })
  ...
  return <>
    ...
    <form onSubmit={formik.handleSubmit} className='md:w-1/2 m-auto py-[20vh]'>
      <h1 className='text-3xl py-4 font-semibold'>Reset Password</h1>
      {/* email */}
      ...
      {/* password */}
      <div className="relative z-0 w-full mb-5 group">
        <input type="password" name="newPassword" onBlur={formik.handleBlur} onChange={formik.handleChange} id="newPassword" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-sky-500 focus:outline-none focus:ring-0 focus:border-sky-600 peer" placeholder=" " />
        <label htmlFor="newPassword" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-sky-600 peer-focus:dark:text-sky-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">enter your newPassword </label>
      </div>
